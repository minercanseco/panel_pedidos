...
from abrir_cajon import CajonCobro
...
class ControladorCobroRapido:
    ...
    def _inicializar_variables_de_instancia(self):
        ...
        self.monto_recibido = self._utilerias.redondear_valor_cantidad_a_decimal(0)
        self.cambio_cliente = self._utilerias.redondear_valor_cantidad_a_decimal(0)
    ...
    def _inicializar_totales_documento(self):
        info_documento = self._base_de_datos.fetchall(
            """
            SELECT 
                CAST(ISNULL(Balance, 0) AS DECIMAL(18,2))     AS Balance,
                CAST(ISNULL(Total, 0) AS DECIMAL(18,2))       AS Total,
                CAST(ISNULL(TotalPaid, 0) AS DECIMAL(18,2))   AS TotalPaid
            FROM docDocument
            WHERE DocumentID = ?
            """,
            (self._document_id,))
    ...
    def _calcular_cambio(self):
        ...
        self.cambio_cliente = cambio_decimal
        self.monto_recibido = monto_recibido_decimal
    ...
    def _guardar_monto_recibido(self):
        ...
        if self.monto_recibido > 0 and self._module_id != 1367:
